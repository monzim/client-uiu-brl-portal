...
function PublicationsList({ publications }: { publications: Publication[] }) {
  const pubs = Array.isArray(publications)
    ? publications
    : (publications as any)?.set || []
          ...
          {years.map((year) => (
            <div key={year}>
              <div className="flex items-center gap-4 mb-6">
                <span className="text-[11px] font-black uppercase tracking-[0.3em] text-brand-text/20">
                  {year}
                </span>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <p className="text-sm font-bold text-brand-accent/70 italic">
                          {pub.venue}
                          {pub.note && (
                            <span className="text-brand-text/40 not-italic font-medium">
                              {' '}
                              — {pub.note}
                            </span>
                          )}
...
function FacultyProfile() {
  ...
  const importantLinks = (
    Array.isArray(faculty.importantLinks)
      ...
      : (faculty.importantLinks as any)?.set || []
  ) as {
    label: string
    url: string
  }[]
          ...
          <div className="lg:col-span-9">
            ...
            <div className="min-h-[500px]">
              ...
              {activeTab === 'publications' && (
                <PublicationsList publications={faculty.publications} />
              )}
            </div>
          </div>
        </div>
