...
function listPublishedByType(memberType: MemberType) {
  return cached(CACHE_KEYS.facultyList(memberType), CACHE_TTL.facultyList, () =>
    db.faculty.findMany({
      where: { published: true, memberType },
...
export const getFacultyList = createServerFn({ method: 'GET' }).handler(
  async (): Promise<DbFaculty[]> => {
    const rows = await listPublishedByType('FACULTY')
    return rows as unknown as DbFaculty[]
  },
)

export const getResearchAssistantList = createServerFn({
  method: 'GET',
}).handler(async (): Promise<DbFaculty[]> => {
  const rows = await listPublishedByType('RESEARCH_ASSISTANT')
  return rows as unknown as DbFaculty[]
})
